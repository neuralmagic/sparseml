""" Align the embeddings of two models"""
import argparse
import json
import pdb 
import logging
...
wandb.init(project='bi-encoder-query-alignment')
...
def main():
    ...
    raw_datasets = load_dataset(args.dataset_name, cache_dir="cache")
    ...
    mimic_model = AutoModel.from_pretrained(
        args.mimic_model_name_or_path,
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
    )
    ...
    loss_fn = nn.MSELoss()
    model = model.to("cuda")
    mimic_model = mimic_model.to("cuda")
    model.eval()
    samples_seen = 0
    eval_loss = []
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            outputs_to_mimic = mimic_model(**batch)
        candidates = torch.cosine_similarity(outputs.pooler_output, outputs_to_mimic.pooler_output)
        loss = loss_fn(candidates,  torch.zeros(candidates.shape).to('cuda'))
        eval_loss.append(loss.item())
    logger.info(f"Eval Loss : np.mean(eval_loss)") 
    wandb.log({"Eval-Loss":np.mean(eval_loss)})
    model.train()
