...
# Sparsifying Bert Model Using Recipes

Introduction about transformers, then Bert (and it's getting into production -- AI 2021 Index report)
Then say that these models are getting bigger, creating challenges for deployment etc
One remedy is model sparsification, lots of interest here (cite eg Ganesh 2020, movement pruning)

Then this tutorial walks the readers through how to sparify a pretrained Bert model on Squad dataset

Also say the results listed in this tutorial are available publicly through a Wandb project

(Where to mention distillation?)

## Set up

Following the installation in the readme, you should have a virtual environment with

## Creating a Pretrained Teacher Model


Before applying one of the pruning recipes, you first create a pretrained model on the Squad dataset.
...
If the command runs successfully, you should have a model folder called `teacher` in the provided model directory `MODELS_DIR`.
...
```bash
python transformers/examples/pytorch/question-answering/run_qa.py \
  --teacher_model_name_or_path MODELS_DIR/teacher \
  --model_name_or_path bert-base-uncased \
  --dataset_name squad \
  --do_train \
  --fp16 \
  ...
  --save_steps 1000 \
  --save_total_limit 2 \
  --recipe ./recipes/sparse80.yaml.md \
  --onnx_export_path MODELS_DIR/sparse80/onnx \
  --report_to wandb
```

### Pruning and dropping layers
