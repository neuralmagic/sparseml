"""
Script to download a PyTorch model from the PyTorch ModelRegistry / Model Repo
...
Download a PyTorch model from the ModelRepo. Use the download command for
downloading
...
##########
Example:
python scripts/pytorch/model_download.py download \
    --model-key resnet50 \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Download a PyTorch model from the ModelRepo. "
...
def main(args):
    ...
    if args.command == DOWNLOAD_COMMAND:
        ...
        LOGGER.info("downloading PyTorch files...")
        pytorch_paths = repo_model.download_framework_files(
        ...
        print("PyTorch: {}".format(pytorch_paths))
