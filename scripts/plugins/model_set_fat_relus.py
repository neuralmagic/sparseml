...
from neuralmagicML.pytorch.recal import convert_relus_to_fat
...
def edit_model(model: Module, state_loaded: bool, **kwargs):
    ...
    """
    fat_convert_after = (
        "init" if "fat_convert_after" not in kwargs else kwargs["fat_convert_after"]
    )

    if (fat_convert_after == "init" and not state_loaded) or (
        fat_convert_after == "load" and state_loaded
    ):
        converted = convert_relus_to_fat(model, inplace=True)
        print(
            "model_set_fat_relus plugin converted {} relus to FAT relus".format(
                len(converted)
            )
        )
