...
def schedule_from_boost_analysis(
    analysis_dir: str,
    losses_per_layer: List[str],
    min_sparsity: float,
    train_epochs: int,
    init_lr: float,
    gamma: float,
    ...
    output = (
        ...
        "\n".format(
            train_epochs,
            ",".join([str(mil) for mil in lr_milestones])
            if len(lr_milestones) > 0
            else train_epochs + 1,
    ...
    losses_per_layer = {
        loss.split("=")[0]: float(loss.split("=")[1]) for loss in losses_per_layer
    }
    ...
    for layer_path in tqdm(paths):
        ...
        for thresh, vals in layer["thresholds"].items():
            layer_vals.append(
                {
                    "losses": {
                        key: numpy.mean(vals["losses"][key]) for key in loss_keys
                    },
        ...
        for layer_val in layer_vals:
            ...
            for loss_key in loss_keys:
                if (
                    baseline["losses"][loss_key] - layer_val["losses"][loss_key]
                    >= losses_per_layer[loss_key]
                ):
                    calc_thresh = numpy.interp(
                        baseline["losses"][loss_key] - losses_per_layer[loss_key],
                        [layer_val["losses"][loss_key], prev_val["losses"][loss_key]],
                        [layer_val["threshold"], prev_val["threshold"]],
                    )
                    calc_sparsity = numpy.interp(
                        baseline["losses"][loss_key] - losses_per_layer[loss_key],
                        [layer_val["losses"][loss_key], prev_val["losses"][loss_key]],
                    ...
                    if limit_thresh is None or calc_thresh < limit_thresh:
                        limit_thresh = (
                            calc_thresh if calc_sparsity >= min_sparsity else -1
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a modifier schedule for boosting from boost analysis"
    )
    ...
    parser.add_argument(
        "--train-epochs", type=int, default=4, help="The number of epochs to train for"
    )
    parser.add_argument(
        "--init-lr", type=float, default=0.005, help="the initial learning rate to use"
    )
    parser.add_argument(
        "--gamma",
        type=float,
