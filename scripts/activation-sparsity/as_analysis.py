...
def as_analysis(device_desc: str, model_type: str, pretrained: Union[bool, str], model_path: Union[None, str],
    ...
    test_dataset, num_classes = create_dataset(dataset_type, dataset_root, train=False, rand_trans=False)
    ...
    with open(csv_path, 'w') as csv_file:
        ...
        for relu_name, relu_sparsities in layer_results['relus'].items():
            csv_writer.writerow(['relu', relu_name, numpy.mean(relu_sparsities), numpy.std(relu_sparsities)])

        for conv_name, conv_sparsities in layer_results['convs'].items():
            csv_writer.writerow(['conv', conv_name, numpy.mean(conv_sparsities), numpy.std(conv_sparsities)])
...
def main():
    ...
    # save options
    parser.add_argument('--save-dir', type=str, default='as-analysis',
