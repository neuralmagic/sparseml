...
def as_boost_analysis(
    ...
    analysis = ModuleASBoostAnalysis(
        model,
        device_desc,
        loss,
        val_dataset,
        ...
        optim_dampening=dampening,
        optim_weight_decay=weight_decay,
        optim_nesterov=nesterov,
    ...
    for layer in relu_layers:
        layer_results = analysis.analyze_layer(
            layer, final_thresh_val=-1.0 if baseline_only else None
        )
...
def _save_layer_results(
    layer: str,
    results: List[Tuple[float, ModuleTestResults, ASAnalyzerLayer]],
    model_dir,
):
    json_data = {
    ...
    for (thresh, test_results, analyzer_layer) in results:
        ...
        json_data["thresholds"][thresh] = {
            ...
            "losses": {
                loss: torch.cat(values).tolist()
                for loss, values in test_results.results.items()
            },
        }
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=2, figsize=size, gridspec_kw={"width_ratios": [1, 1]}
    )
    ...
    loss_axis = axes[1, 0]
    loss_frame = pandas.DataFrame(
        data={"loss": losses[list(losses.keys())[0]]}, index=thresholds
    )
    ...
    sparsity_frame = pandas.DataFrame(
        data={**losses, "sparsity": sparsities}, index=thresholds
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze the boosted activation sparsity for a model "
    ...
    # optimizer settings
    parser.add_argument(
        "--lr", type=float, default=None, help="The learning rate to use while training"
    )
    parser.add_argument(
        "--momentum",
        type=float,
    ...
    as_boost_analysis(
        ...
        args.dataset_root,
        args.sample_size,
        convert_to_bool(args.baseline_only),
        args.train_schedule,
        args.train_size,
