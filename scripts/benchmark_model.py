...
from neuralmagicML.pytorch.utils import ModelBenchmarker, BenchmarkResults
...
def benchmark_model(
    ...
    print(
        "Running benchmarks for batch_sizes:{} and input_sizes:{}".format(
            batch_sizes, input_sizes
        )
    )
    ...
    for batch_size in batch_sizes:
        ...
        res = benchmarker.benchmark_batch(
            batch, full_precision=True, test_size=test_size, warmup_size=warmup_size
        )
...
def _save_results(
    results: List[Tuple[int, BenchmarkResults]],
    output_dir: str,
    model_type: str,
    device: str,
    batch_sizes: List[int],
    ...
    model_id = "{}-{}-{}".format(
        model_type, device, ",".join([str(size) for size in batch_sizes])
    )
    model_tag = model_id
    model_inc = 0
    ...
    save_results = {model_type: batch_results}
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark a model for given batch and input sizes"
    )
