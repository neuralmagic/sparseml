...
def handle_export_command(args):
    ...
    exporter.export_onnx(opset=args.onnx_opset)

    for img_batch, label_batch in train_dataset.take(1):
        output_batch = model(img_batch)
        np.save(os.path.join(args.save_dir, "inputs.npy"), img_batch)
        np.save(os.path.join(args.save_dir, "outputs.npy"), output_batch)
        np.save(os.path.join(args.save_dir, "labels.npy"), label_batch)
