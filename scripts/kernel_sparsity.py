...
class ModelSensitivityAnalysisWidget(object):
    ...
    def __init__(
        self,
        sens_updater: Callable[[List[Tuple[str, float]]], None],
        model: Module,
    ...
    def create(self) -> widgets.Box:
        self._sens_cat_buttons = widgets.ToggleButtons(
            options=[
                ModelSensitivityAnalysisWidget.CAT_UNIFORM,
        ...
        self._eqn_approx_buttons = widgets.ToggleButtons(
            options=[
                ModelSensitivityAnalysisWidget.EQN_APPROX_IS_PARAMS,
        ...
        self._static_approx_device_buttons = {
            device: widgets.ToggleButton(
                value=False, description=device, disabled=False, button_style=""
            )
        ...
        self._static_approx_buttons_container = widgets.HBox(
            (
                ...
                widgets.VBox(
                    tuple(
                        [val for (_, val) in self._static_approx_device_buttons.items()]
                    )
                ),
            )
        )
        self._static_approx_batch_size_slider = widgets.FloatLogSlider(
            value=64, base=2, min=0, max=12, step=1, description="Batch Size:"
        )
        ...
        self._static_approx_cache_check = widgets.Checkbox(
            value=True, description="Cache data in CPU RAM", disabled=False
        )
        self._static_approx_gen_button = widgets.Button(
            description="Run Approx", disabled=False, button_style="info"
        )
        ...
        self._cat_bodies = {
            ...
            ModelSensitivityAnalysisWidget.CAT_STATIC_APPROX: widgets.VBox(
                (
                    self._static_approx_buttons_container,
                    self._static_approx_cache_check,
                    self._static_approx_batch_size_slider,
                    self._static_approx_num_samples_slider,
    ...
    def _update_description(self):
        if self._cat_type == ModelSensitivityAnalysisWidget.CAT_UNIFORM:
            self._desc_text.value = (
                "Uniform distribution, all layers are considered to have the same sensitivity. "
        ...
        if self._cat_type == ModelSensitivityAnalysisWidget.CAT_STATIC_APPROX:
            self._desc_text.value = (
                "Approximate by running a sensitivity analysis independently for each layer "
    ...
    def _update_eqn_approx_description(self):
        if self._eqn_approx_type == ModelSensitivityAnalysisWidget.EQN_APPROX_IS_PARAMS:
            self._eqn_approx_desc_text.value = (
                "Sensitivity based on the number of parameters in each layer "
        ...
        if (
            self._eqn_approx_type
            == ModelSensitivityAnalysisWidget.EQN_APPROX_IS_KERNELS
        ):
            self._eqn_approx_desc_text.value = (
                "Sensitivity based on the convolutional kernel size in each layer "
                "and the amount of change in capacity for the input and output space. "
        ...
        if self._eqn_approx_type == ModelSensitivityAnalysisWidget.EQN_APPROX_ERK:
            self._eqn_approx_desc_text.value = (
                "Sensitivity based on the number of input and output channels "
    ...
    def _run_approx_sensitivities(self):
        if (
            ...
            and self._approx_sensitivities is None
            and self._cat_type != ModelSensitivityAnalysisWidget.CAT_STATIC_APPROX
        ):
            self._approx_sensitivities = approx_sensitivity_analysis(
                self._model, self._sample_input
            )
    ...
    def _run_static_sensitivities(self):
        if (
            ...
            and self._static_sensitivities is None
            and self._cat_type == ModelSensitivityAnalysisWidget.CAT_STATIC_APPROX
        ):
            devices = [
                key
            ...
            device = (
                devices[0]
                if len(devices) < 2
                else "cuda:{}".format(",".join(device[-1] for device in devices))
            )
            ...
            self._static_sensitivities = one_shot_sensitivity_analysis(
                self._model,
                self._dataset,
                self._loss_fn,
                device,
    ...
    def _update_figure(self):
        ...
        if (
            self._cat_type == ModelSensitivityAnalysisWidget.CAT_UNIFORM
            and self._approx_sensitivities
        ):
            sens = [
                (sens.layer_desc.name, sens.uniform)
                for sens in self._approx_sensitivities
            ]

        if (
            self._cat_type == ModelSensitivityAnalysisWidget.CAT_EQN_APPROX
            and self._approx_sensitivities
        ):
            if (
                self._eqn_approx_type
                ...
                sens = [
                    (sens.layer_desc.name, sens.is_params)
                    for sens in self._approx_sensitivities
                ]
            ...
            if (
                ...
                sens = [
                    (sens.layer_desc.name, sens.is_kernels)
                    for sens in self._approx_sensitivities
                ]

            if self._eqn_approx_type == ModelSensitivityAnalysisWidget.EQN_APPROX_ERK:
                sens = [
                    (sens.layer_desc.name, sens.erk)
                    for sens in self._approx_sensitivities
                ]

            if self._eqn_approx_type == ModelSensitivityAnalysisWidget.EQN_APPROX_ER:
                sens = [
                    (sens.layer_desc.name, sens.er)
                    for sens in self._approx_sensitivities
                ]

        if (
            self._cat_type == ModelSensitivityAnalysisWidget.CAT_STATIC_APPROX
            and self._static_sensitivities
        ):
            sens = [
                (sens.layer_desc.name, sens.integral)
                for sens in self._static_sensitivities
            ]
