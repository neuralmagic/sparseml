...
from neuralmagicML.pytorch.utils import convert_to_bool
...
def train_modifiers_schedule(
    ...
    teacher_type: Union[str, None],
    teacher_pretrained: Union[bool, str],
    teacher_path: Union[None, str],
    kd_temp_student: float,
    kd_temp_teacher: float,
    kd_weight: float,
    kd_contradict_hinton: bool,
    dataset_type: str,
    dataset_root: str,
    ...
    nesterov: bool,
    save_dir: str,
    save_after_epoch: int,
    save_epochs: Union[None, List[int]],
    save_epoch_mod: int,
    track_ks: bool,
    track_as: bool,
    track_gradients: Union[None, List[str]],
    ...
    def _create_data_loader(_dataset: Dataset, _train: bool):
        return DataLoader(
            _dataset,
            batch_size=train_batch_size if _train else test_batch_size,
            shuffle=_train,
    ...
    train_test_dataset = EarlyStopDataset(
        ...
        else round(0.2 * len(train_dataset)),
    )
    ...
    if teacher_type is not None:
        ...
        kd_settings = KDSettings(
            teacher, kd_temp_student, kd_temp_teacher, kd_weight, kd_contradict_hinton
        )
        print(
            "Created teacher model of type {} with num_classes:{}, pretrained:{} / model_path:{}".format(
                teacher_type, num_classes, teacher_pretrained, teacher_path
            )
        )
        print(
            "Created kd settings with teacher model of temp_student:{}, "
            "temp_teacher:{}, weight:{}, con_hinton:{}".format(
                kd_temp_student, kd_temp_teacher, kd_weight, kd_contradict_hinton
            )
        )
    else:
        teacher = None
    ...
    optimizer = SGD(
        model.parameters(), init_lr, momentum, dampening, weight_decay, nesterov
    )
    ...
    print(
        "Created optimizer with init_lr:{} momentum:{} dampening:{} weight_decay:{} nesterov:{}".format(
            init_lr, momentum, dampening, weight_decay, nesterov
        )
    )
    ...
    model_tag = "{}_{}-{}-{}".format(
        model_type.replace("/", "."),
        teacher_type.replace("/", ".") if teacher_type else "None",
        dataset_type,
    ...
    logs_dir = os.path.abspath(
        os.path.expanduser(os.path.join(save_dir, "tensorboard-logs"))
    )
            ...
            for name, mod in model.named_modules():
                for param_name, param in mod.named_parameters():
                    ...
                    if "norm" in track_gradients:
                        writer.add_scalar(
                            "Gradients/{}/{}/norm".format(name, param_name),
                            grad.abs().sum(),
                            _step,
                        )
                    ...
                    if "mean" in track_gradients:
                        writer.add_scalar(
                            "Gradients/{}/{}/mean".format(name, param_name),
                            grad.mean(),
                            _step,
                        )
                    ...
                    if "var" in track_gradients:
                        writer.add_scalar(
                            "Gradients/{}/{}/var".format(name, param_name),
                            grad.var(),
                            _step,
                        )
                    ...
                    if "hist" in track_gradients:
                        writer.add_histogram(
                            "Gradients/{}/{}/hist".format(name, param_name), grad, _step
                        )
    ...
    ks_analyzers = ModuleKSAnalyzer.analyze_layers(
        model, analyze_layers
    )  # type: List[ModuleKSAnalyzer]
    as_analyzers = ModuleASAnalyzer.analyze_layers(
        model, analyze_layers, division=None, track_inputs_sparsity=True
    )  # type: List[ModuleASAnalyzer]
    as_dataset = EarlyStopDataset(
        train_test_dataset, early_stop=1000 if len(train_test_dataset) > 1000 else -1
    )
    ...
    def _run_model_tests(_epoch: int):
        ...
        for _loss in _val_results.results.keys():
            writer.add_scalar(
                "Test/Validation/{}".format(_loss),
                _val_results.result_mean(_loss),
                _epoch,
            )
            print(
                "Epoch {} Test/Validation/{}: {}".format(
                    _epoch, _loss, _val_results.result_mean(_loss)
                )
            )

        print("Testing training dataset for epoch {}".format(_epoch))
        _train_results = tester.run_epoch(
            _create_data_loader(train_test_dataset, False), _epoch
        )

        for _loss in _train_results.results.keys():
            writer.add_scalar(
                "Test/Training/{}".format(_loss),
                _train_results.result_mean(_loss),
                _epoch,
            )
            print(
                "Epoch {} Test/Training/{}: {}".format(
                    _epoch, _loss, _train_results.result_mean(_loss)
                )
            )

        if track_as:
            print(
                "Testing activation sparsity for training dataset for epoch {}".format(
                    _epoch
                )
            )
            ...
            for _index, _analyzer in enumerate(as_analyzers):
                ...
                writer.add_scalar(
                    "Act Sparsity/{}".format(
                        analyze_layers[_index].replace("module.", "")
                    ),
                    _analyzer.inputs_sparsity_mean,
                    epoch,
                )
        ...
        if track_ks:
            for _ks_layer in ks_analyzers:
                writer.add_scalar(
                    "Kernel Sparsity/{}".format(_ks_layer.name.replace("module.", "")),
                    _ks_layer.param_sparsity,
                    epoch,
                )
    ...
    while epoch < modifier_manager.max_epochs - 1:
        ...
        if (epoch in save_epochs) or (
            epoch >= save_after_epoch
            and save_epoch_mod > 0
            and epoch % save_epoch_mod == 0
        ):
            save_path = os.path.join(
                save_dir, "{}.checkpoint-{:03d}.pth".format(model_id, epoch)
            )
...
def main():
    ...
    train_modifiers_schedule(
        ...
        args.teacher_path,
        args.kd_temp_student,
        args.kd_temp_teacher,
        args.kd_weight,
        convert_to_bool(args.kd_contradict_hinton),
        args.dataset_type,
        args.dataset_root,
        ...
        convert_to_bool(args.load_optim),
        args.init_lr,
        args.momentum,
        args.dampening,
        args.weight_decay,
        convert_to_bool(args.nesterov),
        args.save_dir,
        args.save_after_epoch,
        args.save_epochs,
        args.save_epoch_mod,
        convert_to_bool(args.track_ks),
        convert_to_bool(args.track_as),
        args.track_grads,
