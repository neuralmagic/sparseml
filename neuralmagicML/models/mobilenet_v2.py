...
def _make_divisible(value: float, divisor: int, min_value: Union[int, None] = None) -> int:
...
class _InvertedResidualBlock(Module):
    def __init__(self, in_channels: int, out_channels: int, exp_channels: int, stride: int,
...
class MobilenetV2SectionSettings(object):
    def __init__(self, num_blocks: int, in_channels: int, out_channels: int, downsample: bool,
                 exp_channels: Union[None, int] = None, exp_ratio: float = 1.0,
                 init_section: bool = False, width_mult: float = 1.0):
        self.num_blocks = num_blocks
        self.in_channels = _make_divisible(in_channels * width_mult, 8) if not init_section else in_channels
...
class MobilenetV2(Module):
    ...
    def create_section(settings: MobilenetV2SectionSettings) -> Sequential:
        ...
        for _ in range(settings.num_blocks):
            if apply_exp_kwargs:
                blocks.append(_InvertedResidualBlock(
...
def mobilenet_v2(width_mult, model_arch_tag: str, **kwargs) -> MobilenetV2:
    sec_settings = [
        MobilenetV2SectionSettings(num_blocks=1, in_channels=3, out_channels=16, exp_channels=32,
                                   downsample=False, init_section=True, width_mult=width_mult),
        MobilenetV2SectionSettings(num_blocks=2, in_channels=16, out_channels=24, exp_ratio=6,
                                   downsample=True, init_section=False, width_mult=width_mult),
        MobilenetV2SectionSettings(num_blocks=3, in_channels=24, out_channels=32, exp_ratio=6,
                                   downsample=True, init_section=False, width_mult=width_mult),
        MobilenetV2SectionSettings(num_blocks=4, in_channels=32, out_channels=64, exp_ratio=6,
                                   downsample=True, init_section=False, width_mult=width_mult),
        MobilenetV2SectionSettings(num_blocks=3, in_channels=64, out_channels=96, exp_ratio=6,
                                   downsample=False, init_section=False, width_mult=width_mult),
        MobilenetV2SectionSettings(num_blocks=3, in_channels=96, out_channels=160, exp_ratio=6,
                                   downsample=True, init_section=False, width_mult=width_mult),
        MobilenetV2SectionSettings(num_blocks=1, in_channels=160, out_channels=320, exp_ratio=6,
                                   downsample=False, init_section=False, width_mult=width_mult)
    ]
