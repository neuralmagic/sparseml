...
class UnstructuredSparsityMaskCreator(SparsityMaskCreator):
    """
...
class GroupedSparsityMaskCreator(UnstructuredSparsityMaskCreator):
    ...
    def get_grouping_fn(grouping_fn_name: str) -> Callable[[Tensor, List[int]], Tensor]:
        ...
        """
        if grouping_fn_name not in GroupedSparsityMaskCreator._GROUPING_FUNCTIONS:
            raise ValueError(
                "Invalid grouping fn {}, valid grouping fns: {}".format(
                    grouping_fn_name, GroupedSparsityMaskCreator._GROUPING_FUNCTIONS
                )
            )
...
class DimensionSparsityMaskCreator(GroupedSparsityMaskCreator):
    """
...
class BlockSparsityMaskCreator(GroupedSparsityMaskCreator):
    """
...
mask_creator_name_to_constructor_lambda = {
    "unstructured": lambda: UnstructuredSparsityMaskCreator(),
    "channel": lambda: DimensionSparsityMaskCreator("channel"),
    "filter": lambda: DimensionSparsityMaskCreator("filter"),
}


def load_mask_creator(obj: Union[str, Iterable[int]]) -> SparsityMaskCreator:
    """
