...
class OpenImagesDataset(Dataset):
    ...
    TRAINABLE_CLASSES = 'https://storage.googleapis.com/openimages/v5/classes-trainable.txt'
    ...
    def __init__(self, root: str, train: bool = True, rand_trans: bool = False,
                 confidence: float = 1.0, image_size: int = 224, samples_num_workers: int = 0,
                 download_num_workers: int = 0, download_sample_size: int = -1, download_image_size: int = 600,
                 download_test_images: bool = False):
        print('OpenImagesDataset: checking download')
        OpenImagesDataset.download(root, download_num_workers, download_sample_size, download_image_size,
        ...
        self.samples = OpenImagesDataset.get_split_samples(split, root, confidence, samples_num_workers)

        self.trainable_classes_lock = Lock()
        self.trainable_classes = pandas.read_csv(
            os.path.join(root, 'meta', os.path.basename(OpenImagesDataset.TRAINABLE_CLASSES)), names=['LabelName']
        )
    ...
    def __getitem__(self, index):
        ...
        with self.trainable_classes_lock:
            label_names = self.trainable_classes.copy()['LabelName'].sort_values().tolist()
    ...
    @staticmethod
    def get_split_samples(split: str, root: str, confidence: float, num_workers: int) -> List[Tuple[str, str, str]]:
        ...
        def _samples_worker(_vals: Tuple[str, float]) -> Union[None, Tuple[str, str, str]]:
            _file_path, _confidence = _vals
            _human_only = _confidence >= (1.0 - sys.float_info.epsilon) or _confidence < 0.0
            ...
            _human_path = os.path.join(_samples_path, '{}.lab-human.json'.format(_image_id))
            _machine_path = os.path.join(_samples_path, '{}.lab-machine.json'.format(_image_id))
            ...
            if _image_path is not None and ((_machine_path is not None and not _human_only) or _human_path is not None):
        ...
        for vals in tqdm(worker, desc='collecting data samples for {}...'.format(split), total=num_files):
            if vals is None:
                continue
    ...
    @staticmethod
    def download(root_dir: str, num_workers: int, sample_size: int, image_size: int, test_images: bool):
        try:
        ...
        if os.path.exists(downloaded_file_path):
            print('Open Images already downloaded in {} , delete the folder to redownload'.format(root_dir))
        ...
        OpenImagesDataset._download_images(root_dir, meta_dir, num_workers, sample_size, image_size, test_images)
    ...
    def _download_supporting_files(meta_dir: str, num_workers: int):
        supporting_sources = [
            OpenImagesDataset.CLASS_NAMES, OpenImagesDataset.TRAINABLE_CLASSES,
            OpenImagesDataset.TRAIN_HUMAN_LABELS, OpenImagesDataset.TRAIN_MACHINE_LABELS,
            OpenImagesDataset.TRAIN_IMAGE_IDS,
            OpenImagesDataset.VAL_HUMAN_LABELS, OpenImagesDataset.VAL_MACHINE_LABELS,
            OpenImagesDataset.VAL_IMAGE_IDS,
            OpenImagesDataset.TEST_HUMAN_LABELS, OpenImagesDataset.TEST_MACHINE_LABELS,
            OpenImagesDataset.TEST_IMAGE_IDS
        ]
        source_dests = []

        for source in supporting_sources:
            source_dests.append((os.path.basename(source), source, os.path.join(meta_dir, os.path.basename(source))))
        ...
        for download in tqdm(downloader, desc='downloading supporting files'):
            if download.err:
                raise RuntimeError('Exception while downloading {} from {} to {} : {}'
                                   .format(os.path.basename(download.source), download.source, download.dest,
                                           download.err))
    ...
    @staticmethod
    def _download_images(root_dir: str, meta_dir: str, num_workers: int, sample_size: int,
                         image_size: int, test_images: bool):
        OpenImagesDataset._download_split_images(
            'validation', os.path.join(meta_dir, os.path.basename(OpenImagesDataset.VAL_IMAGE_IDS)),
            os.path.join(meta_dir, os.path.basename(OpenImagesDataset.VAL_HUMAN_LABELS)),
            os.path.join(meta_dir, os.path.basename(OpenImagesDataset.VAL_MACHINE_LABELS)),
            root_dir, num_workers, sample_size, image_size
        )
        OpenImagesDataset._download_split_images(
            'train', os.path.join(meta_dir, os.path.basename(OpenImagesDataset.TRAIN_IMAGE_IDS)),
            os.path.join(meta_dir, os.path.basename(OpenImagesDataset.TRAIN_HUMAN_LABELS)),
            os.path.join(meta_dir, os.path.basename(OpenImagesDataset.TRAIN_MACHINE_LABELS)),
            root_dir, num_workers, sample_size, image_size
        )

        if test_images:
            OpenImagesDataset._download_split_images(
                'test', os.path.join(meta_dir, os.path.basename(OpenImagesDataset.TEST_IMAGE_IDS)),
                os.path.join(meta_dir, os.path.basename(OpenImagesDataset.TEST_HUMAN_LABELS)),
                os.path.join(meta_dir, os.path.basename(OpenImagesDataset.TEST_MACHINE_LABELS)),
                root_dir, num_workers, sample_size, image_size
            )

    @staticmethod
    def _download_split_images(split_name: str, meta_path: str, human_labels_path: str, machine_labels_path: str,
                               root_dir: str, num_workers: int, sample_size: int, image_size: int):
        print('starting download for {}'.format(split_name))
        images_dir = os.path.join(root_dir, split_name)
        downloader = _ImageDownloader(split_name, images_dir, meta_path, num_workers, sample_size)
...
class _ImageDownloader(object):
    def __init__(self, split_name: str, images_dir: str, meta_path: str,
                 num_workers: int = 0, sample_size: int = -1, image_size: int = 600):
    ...
    def download(self) -> Iterator[DownloadResult]:
        ...
        with open(self._meta_path) as meta_file:
            ...
            for row in meta_csv:
                ...
                image_id = row[0]
                image_url = row[2]
                source_dests.append((image_id, image_url, os.path.join(self._images_dir, '{}.jpg'.format(image_id))))

                if len(source_dests) >= self._sample_size > 0:
                    break

        downloader = MultiDownloader(source_dests, downloaded_callback=self._resize, num_workers=self._num_workers)

        for download in tqdm(downloader, desc='downloading {} images'.format(self._split_name)):
            yield download
...
class _ImageLabeler(object):
    def __init__(self, split_name: str, images_dir: str, human_labels_path: str, machine_labels_path: str,
                 num_workers: int = 0, image_size: int = 600):
        if num_workers < 1:
            num_workers = multiprocessing.cpu_count()  # scale with the number of cores on the machine
    ...
    def label(self):
        ...
        labels = [('machine', self._machine_labels_path), ('human', self._human_labels_path)]

        for (labels_type, labels_path) in labels:
            print('parsing {} labels for {}...'.format(labels_type, self._split_name))
            label_worker = ParallelWorker(_ImageLabeler._create_label, self._num_workers,
                                          indefinite=True, max_source_size=100)
            label_worker.start()
            ...
            with open(labels_path, 'r+') as labels_file:
                ...
                for line in tqdm(labels_reader,
                                 desc='creating {} labels for {}...'.format(labels_type, self._split_name),
                    ...
                    if image_id != line[0]:
                        if image_id is not None:
                            label_worker.add_item((image_id, labels_type, labels, self._images_dir))
            ...
            print('finishing up {} labels for {}...'.format(labels_type, self._split_name))
    ...
    def _create_label(info: Tuple[str, str, Dict[str, float], str]):
        ...
        label_path = os.path.join(images_dir, '{}.lab-{}.json'.format(image_id, label_type))
