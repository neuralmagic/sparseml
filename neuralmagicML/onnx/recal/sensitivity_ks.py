...
) -> Generator[Tuple[KSLossSensitivityAnalysis, KSSensitivityProgress], None, None]:
    """
...
def approx_ks_loss_sensitivity(
    ...
    for (analysis, progress) in approx_ks_loss_sensitivity_iter(model, sparsity_levels):
...
def one_shot_ks_loss_sensitivity_iter(
    model: Union[str, ModelProto],
    data: DataLoader,
    batch_size: int,
...
def one_shot_ks_loss_sensitivity(
    ...
    for (analysis, progress) in one_shot_ks_loss_sensitivity_iter(
        model,
        data,
        batch_size,
...
def one_shot_ks_perf_sensitivity_iter(
    model: Union[str, ModelProto],
    data: DataLoader,
    batch_size: int,
    num_cores: int = -1,
...
) -> Generator[Tuple[KSPerfSensitivityAnalysis, KSSensitivityProgress], None, None]:
    """
    Run a one shot sensitivity analysis for kernel sparsity.
