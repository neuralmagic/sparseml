...
from neuralmagicML.frameworks import (
    ONNX_FRAMEWORK,
    KERAS_FRAMEWORK,
    PYTORCH_FRAMEWORK,
...
REPO_BASE_URI = "https://models.neuralmagic.com"
SIGNATURE_URI = "https://api.neuralmagic.com/models/sign-url"
NM_SIGNING_TOKEN = (
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9."
    "eyJhY2Nlc3MiOiJhbGwiLCJleHAiOjE5NDQ0OTUwNDMsInVzZXJJZCI6IjEyMzQ1NiIsImFwcElkIjoiNzg5MDEyIn0."
    "G4LlNb7DOqn1oxkKELTtvgR_laeYDM22YpJSgmP0Zkl82g5nqiGpHg9hegABPiUPCaTpqFaKRviDmaVZO6OeiiB3aXTl"
    "fYTYL26X0GJfcTK1l3O7PouxYrlN35l31rgYtTk4qyIPH1o6JmNRwuxIe1N_OZ2Vi7XQk46HmyS1GuhOOKC1UG3IoPPT9"
    "bxIF5uepZjQYN3bNQ-QmRi_ldWPe1Pl-kXD59H31PkjCNzhnVqt_FQI27lLgjDdhhbqHMzrUrOqXnR3WsJTkIajrWDFbM"
    "WKI9OJuAcaMATkWoO2ssUH7ADMYAR5B3bWbZKlXPZjWIjbNhIJANJNVg_F5i6OH4xVvqdLWnzNkmBShKwOCnFGTpRjCEg"
    "ISHgfwl9CyTT5GT8XJA6VFaNFmHyK-TLWSCwir70eRRwUkApBva0KA51VAIv5YW2zmpscyWGHsDS8OTi0Ry7PZSbssJFv"
    "7zke7xY95NH54aMP1BbpIHwDPz1UNYtqp1WLZvE_TvGW3JsYTasZiOhaNYrlc1hssaxZM2rMccaV5I8XVyE5jIn1jCgkw"
    "-7ETTklN-440FLeCsEbZTm5g3Zmw0nE35AfR1uawpCGa0hx2lH0Q59JTSVgQRJmOiYw0XpINsL3OznzyaOxJSIfzQbgq6"
    "3nNCaqcW9uM1pOyoNPYPHvlTqkaZMh3pE"
)
...
def models_sign_url(path: str) -> str:
    ...
    """
    headers = {"nm-token-header": NM_SIGNING_TOKEN}
    model_req_data = {"body": {"redirect_path": path}}
    model_req_data = json.dumps(model_req_data)
    request = requests.post(url=SIGNATURE_URI, data=model_req_data, headers=headers)
...
def models_download_file(path: str, overwrite: bool, overwrite_path: str = None) -> str:
    ...
    if overwrite and os.path.exists(save_file):
        try:
            ...
            print(
                "warning, error encountered when removing older cache_file at {}: {}".format(
                    save_file, err
                )
            )
...
class RepoModel(object):
    ...
    def __init__(
        ...
        :param dataset: the dataset used to train the model; ex: imagenet, cifar, etc
        :param framework: the framework used to train the model; ex: tensorflow, pytorch, keras, onnx, etc
        :param desc: the description of the model; ex: base, recal, recal_perf
    ...
    def root_path(self) -> str:
        """
        :return: the combined root path the model will be found at in models.neuralmagic.com
        """
    ...
    def onnx_file_path(self) -> str:
        """
        :return: path the onnx file for the model will be found at in models.neuralmagic.com
        """
    ...
    def framework_file_path(self):
        """
        :return: path the framework specific file for the model will be found at in models.neuralmagic.com
        """
...
def filter_model(
    ...
    :param sub_domains: the sub domains the model must belong to; ex: classification, detection, etc
    :param architectures: the architecture the model must belong to; ex: resnet, mobilenet, etc
    :param sub_architectures: the sub architectures the model must belong to; ex: 50, 101, etc
    :param datasets: the datasets the model must have been trained on; ex: imagenet, cifar, etc
    :param frameworks: the frameworks the model must have been trained in; ex: tensorflow, pytorch, etc
    :param descs: the descriptions the model must have; ex: base, recal, recal_perf
...
def available_models(
    ...
    if AVAILABLE_MODELS is None:
        ...
        for mod in models_dicts:
            AVAILABLE_MODELS.append(RepoModel(**mod))
