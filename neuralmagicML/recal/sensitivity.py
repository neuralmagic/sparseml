...
def default_check_sparsities_loss(extended: bool) -> Tuple[float, ...]:
    """
...
def default_check_sparsities_perf() -> Tuple[float, ...]:
    """
    :return: the sparsities to check for effects on the loss
    """
...
class KSSensitivityResult(object):
    """
...
class KSLossSensitivityAnalysis(object):
    ...
    def __init__(self):
        self._results = []  # type: List[KSSensitivityResult]
    ...
    def add_result(
        ...
        if len(result) > 0:
            ...
            result = KSSensitivityResult(id_, name, index)
            self._results.append(result)

        result.add_measurement(sparsity, measurement, baseline)

    def get_result(self, id_or_name: str) -> KSSensitivityResult:
        """
...
class KSPerfSensitivityAnalysis(object):
    """
    Analysis result for how kernel sparsity (pruning) affects the loss of a given model.
    Contains layer by layer results.
    ...
    def __init__(self, num_cores: int, batch_size: int):
        ...
        self._results = []  # type: List[KSSensitivityResult]
        self._results_model = KSSensitivityResult("__model__", "model", -1)
