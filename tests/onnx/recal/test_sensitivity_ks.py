...
import pytest
from neuralmagicML.onnx.recal.sensitivity_ks import (
...
def _create_sensitivity_ks_data(
    ...
    analysis = approx_ks_loss_sensitivity(model_path)
    ...
    if neuralmagic is not None:
        analysis = one_shot_ks_perf_sensitivity(
            model_path,
            dataloader,
            1,
            -1,
...
def test_approx_ks_loss_sensitivity(
    ...
    analysis = approx_ks_loss_sensitivity(onnx_models_with_analysis.model_path)

    expected_analysis = KSLossSensitivityAnalysis.load_json(
