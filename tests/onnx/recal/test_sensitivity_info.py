...
from neuralmagicML.onnx.recal.sensitivity_info import (
    SensitivityModelInfo,
    SensitivityNodeInfo,
)
from neuralmagicML.recal.sensitivity import (
...
NodeInfoFixture = NamedTuple(
    ...
    [
        ("node", NodeAnalyzer),
        ("loss_result", Dict[str, KSSensitivityResult]),
        ("perf_result", Dict[str, KSSensitivityResult]),
...
def node_info(request) -> NodeInfoFixture:
    ...
    loss_result = KSSensitivityResult.from_dict(loss_result)
    perf_result = KSSensitivityResult.from_dict(perf_result)
...
def _load_sensitivity_info_fields_from_json(
    ...
    perf_analysis = OrderedDict(
        [
            (key, KSPerfSensitivityAnalysis.from_dict(analysis))
    ...
    loss_analysis = OrderedDict(
        [
            (key, KSLossSensitivityAnalysis.from_dict(analysis))
