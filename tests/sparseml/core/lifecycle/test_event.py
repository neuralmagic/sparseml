...
from sparseml.core.lifecycle.event import EventLifecycle, WrappedOptimEventLifecycle
import pytest
...
def _get_event_lifecycle(start=None, type_first=None, lifecycle_class=EventLifecycleDummyChild):
...
class TestEventLifecycle:
    ...
    def test_init(self):
        ...
        lifecycle = _get_event_lifecycle(
            type_first=type_first, start=start
            )
    ...
    @pytest.mark.parametrize( "type_, expected_func_name", [
    ...
    @pytest.mark.parametrize("kwargs, increment ,expected", [
...
class TestWrappedOptimEventLifecycle:
    ...
    def test_wrapped_lifecycle_raises_value_error_on_erroneous_invocation(self, method_name):
        lifecycle = _get_event_lifecycle(lifecycle_class=WrappedOptimEventLifecycle)
        with pytest.raises(ValueError, match="batch"):
            method = getattr(lifecycle, method_name)
            method()
            
    @pytest.mark.parametrize("type_first", [
        EventType.BATCH_START,
    ])
    def test_loss_calculated_events_with_wrong_type_first(self, type_first):
        lifecycle = _get_event_lifecycle(type_first=type_first, lifecycle_class=WrappedOptimEventLifecycle)
    ...
    def test_loss_calculated_events_with_wrong_type_(self, type_):
        lifecycle = _get_event_lifecycle(lifecycle_class=WrappedOptimEventLifecycle, type_first=EventType.LOSS_CALCULATED)
    ...
    def test_loss_calculated_events(self, monkeypatch, check_step_batches_count_return):
        ...
        monkeypatch.setattr(lifecycle, "check_step_batches_count", partial(mock_check_step_batches_count, ret=check_step_batches_count_return))
    ...
    @pytest.mark.parametrize("type_first, type_", [
    ...
    def test_optim_pre_step_events_raises_value_error(self, type_first, type_):
        lifecycle = _get_event_lifecycle(lifecycle_class=WrappedOptimEventLifecycle, type_first=type_first)
        lifecycle.type_ = type_
        
        with pytest.raises(ValueError, match="optim pre step must"):
            lifecycle.optim_pre_step_events()
            
    @pytest.mark.parametrize("type_first, type_, check_step_invocations_count_return", [
    ...
    def test_optim_pre_step_events(self, type_first, type_, check_step_invocations_count_return, monkeypatch):
        lifecycle = _get_event_lifecycle(lifecycle_class=WrappedOptimEventLifecycle, type_first=type_first)
        lifecycle.type_ = type_
        def mock_check_step_invocations_count(ret=True, *args, **kwargs):
            return ret
        
        monkeypatch.setattr(lifecycle, "check_step_invocations_count", partial(mock_check_step_invocations_count, ret=check_step_invocations_count_return))
    ...
    @pytest.mark.parametrize("type_, check_step_invocations_count_return", [
    (EventType.OPTIM_PRE_STEP, False),
    ])
    def test_optim_post_step_events(self, type_, monkeypatch, check_step_invocations_count_return):
        lifecycle = _get_event_lifecycle(lifecycle_class=WrappedOptimEventLifecycle)
        lifecycle.type_ = type_
