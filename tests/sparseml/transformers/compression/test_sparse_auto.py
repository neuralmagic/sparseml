...
from sparseml.transformers.compression import SparsityConfigFiller
...
def test_sparse_model_reload(compressed, config, dtype, tmp_path):
    ...
    inferred_global_sparsity = SparsityConfigFiller.infer_global_sparsity(model)
    assert math.isclose(inferred_global_sparsity, 19.6562, rel_tol=1e-3)
    inferred_structure = SparsityConfigFiller.infer_sparsity_structure()
