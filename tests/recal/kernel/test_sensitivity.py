...
from neuralmagicML.recal import ModuleParamKSSensitivity, KSSensitivityProgress, ModuleKSSensitivityAnalysis
from neuralmagicML.utils import LossWrapper


@pytest.mark.parametrize(
    'name,param_name,type_,execution_order,measured,expected_integral',
    [
        ('layer.name', 'weight', 'linear', 0, [(0.05, 0.0), (0.25, 0.0), (0.5, 0.0), (0.75, 0.0), (0.95, 0.0)], 0.0),
        ('layer.name', 'weight', 'conv', 0, [(0.05, 1.0), (0.25, 1.0), (0.5, 1.0), (0.75, 1.0), (0.95, 1.0)], 1.0),
        ('layer.name', 'weight', 'conv', 0, [(0.05, 1.0), (0.25, 1.5), (0.5, 2.0), (0.75, 3.75), (0.95, 6.0)], 2.7312498092651367),
    ]
)
@pytest.mark.device_cpu
def test_module_param_ks_sensitivity(name, param_name, type_, execution_order, measured, expected_integral):
...
@pytest.mark.parametrize(
    ...
    [
        (0, 'layer.name', ['layer.name', 'layer2.name'], 0, [0.2, 0.5, 0.7, 0.9, 0.95], 0, 100)
    ]
)
@pytest.mark.device_cpu
def test_ks_sensitivity_progress(layer_index, layer_name, layers, sparsity_index, sparsity_levels, measurement_step, samples_per_measurement):
    progress = KSSensitivityProgress(layer_index, layer_name, layers, sparsity_index, sparsity_levels, measurement_step, samples_per_measurement)
...
def _test_module_ks_sensitivity_analysis_one_shot(model, dataset, loss_fn, batch_size, samples_per_measurement, device):
    ...
    expected = ((1 + math.ceil(samples_per_measurement / batch_size)) * len(results[-1].measured) + 1) * len(results)
...
@pytest.mark.device_cpu
def test_module_ks_sensitivity_analysis_one_shot(model, dataset, loss_fn, batch_size, samples_per_measurement):
    _test_module_ks_sensitivity_analysis_one_shot(model, dataset, loss_fn, batch_size, samples_per_measurement, 'cpu')
...
def test_module_ks_sensitivity_analysis_one_shot_cuda(model, dataset, loss_fn, batch_size, samples_per_measurement):
    _test_module_ks_sensitivity_analysis_one_shot(model, dataset, loss_fn, batch_size, samples_per_measurement, 'cuda')
