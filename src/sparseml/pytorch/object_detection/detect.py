...
def run(
    ...
    project=LOCAL_ROOT / "runs/detect",  # save results to project/name
    ...
    LOGGER.info(
        f"Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape "
        f"{(1, 3, *imgsz)}" % t
    )
...
def parse_opt():
    ...
    parser.add_argument(
        "--project",
        default=LOCAL_ROOT / "runs/detect",
