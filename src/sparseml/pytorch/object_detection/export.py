...
def export_engine(
    ...
    try:
        ...
        assert (
            im.device.type != "cpu"
        ), "export running on CPU but must be on GPU, i.e. `python export.py --device 0`"  # noqa
        ...
        # fix TRT 8.4 deprecation notice
        ...
        for inp in inputs:
            LOGGER.info(
                f'{prefix}\tinput "{inp.name}" with shape {inp.shape} and '
                f"dtype {inp.dtype}"
            )
        for out in outputs:
            LOGGER.info(
                f'{prefix}\toutput "{out.name}" with shape {out.shape} and '
                f"dtype {out.dtype}"
            )

        LOGGER.info(
            f"{prefix} building FP{16 if builder.platform_has_fast_fp16 else 32} "
            f"engine in {f}"
        )
...
def export_edgetpu(keras_model, im, file, prefix=colorstr("Edge TPU:")):
    ...
    try:
        ...
        if subprocess.run(cmd + " >/dev/null", shell=True).returncode != 0:
            LOGGER.info(
                f"\n{prefix} export requires Edge TPU compiler. Attempting install "
                f"from {help_url}"
            )
            sudo = (
...
def export_tfjs(keras_model, im, file, prefix=colorstr("TensorFlow.js:")):
    ...
    try:
        ...
        cmd = (
            ...
            f'--output_node_names="Identity,Identity_1,Identity_2,Identity_3" {f_pb} '
            f"{f}"
        )
...
def run(
    ...
    if any((saved_model, pb, tflite, edgetpu, tfjs)):
        ...
        assert not (tflite and tfjs), (
            "TFLite and TF.js models must be exported separately, please pass only "
            "one type."
        )
        ...
        if tflite or edgetpu:
            f[7] = export_tflite(
                model, im, file, int8=int8 or edgetpu, data=data, ncalib=100
            )
        if edgetpu:
...
def parse_opt():
    ...
    parser.add_argument(
        ...
        help=(
            "torchscript, onnx, openvino, engine, coreml, saved_model, pb, "
            "tflite, edgetpu, tfjs"
        ),
    )
