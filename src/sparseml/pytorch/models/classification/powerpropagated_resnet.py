...
def _init_conv(conv: Conv2d, alpha=2):
    init.kaiming_normal_(conv.weight, mode="fan_out", nonlinearity="relu")
    conv.weight = Parameter(conv.weight*pow(abs(conv.weight), 1/alpha-1))
...
def _init_linear(linear: Linear, alpha=2):
    init.normal_(linear.weight, 0, 0.01)
    linear.weight = Parameter(linear.weight*pow(abs(linear.weight), 1/alpha-1))
...
class PowerpropagatedConv2d(Conv2d):
    def _conv_forward(self, input, weight, bias):
        alpha = 2  #TODO: make this configurable
        powerpropagated_weight = weight *pow(abs(weight), alpha-1)
        return super()._conv_forward(input, powerpropagated_weight, bias)
...
class _Input(Module):
    ...
    def __init__(self):
        super().__init__()
        self.conv = PowerpropagatedConv2d(
            _Input.IN_CHANNELS,
            _Input.OUT_CHANNELS,
            kernel_size=7,
            stride=2,
...
class _BasicBlock(Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1):
        super().__init__()
        self.conv1 = PowerpropagatedConv2d(
            in_channels,
            out_channels,
            kernel_size=3,
            stride=stride,
        ...
        self.conv2 = PowerpropagatedConv2d(
...
class _BottleneckBlock(Module):
    def __init__(
        ...
        self.conv1 = PowerpropagatedConv2d(in_channels, proj_channels, kernel_size=1, bias=False)
        ...
        self.conv3 = PowerpropagatedConv2d(proj_channels, out_channels, kernel_size=1, bias=False)
...
class _BasicBlockV2(Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1):
        ...
        self.identity = (
            PowerpropagatedConv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)
...
class _Classifier(Module):
    def __init__(self, in_channels: int, num_classes: int, class_type: str = "single"):
        ...
        self.fc = PowerpropagatedLinear(in_channels, num_classes)
