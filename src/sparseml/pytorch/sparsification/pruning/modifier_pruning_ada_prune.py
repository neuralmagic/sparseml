...
__all__ = [
    "AdaPruneMagnitudePruningModifier",
    "AdaPruneGlobalMagnitudePruningModifier",
    "AdaPruneMFACPruningModifier",
    "AdaPruneMovementPruningModifier",
]
...
class AdaPruneBaseModifier(BaseGradualPruningModifier):
    ...
    def initialize(
        ...
        teacher_module : Module = None,
        calibration_images: Tensor = None,
        **kwargs,
    ):
        """
        ...
        if teacher_module is not None:
            ...
            _LOGGER.debug("Using the copy of the provided module as teacher")
        ...
        for submodule_name, submodule in module.named_modules():
            if re.search(self._modules, submodule_name):
                self._modules_dict[submodule_name] = submodule
        ...
        self._optimizer_dict = {
            ...
            for submodule_name, submodule in self._modules_dict.items()
        }
        # init parent class
    ...
    def calibrate(self):
        ...
        hooks = {}
        for submodule_name, submodule in self._teacher_module.named_modules():
            if re.search(self._modules, submodule_name):
                ...
                hooks[submodule_name] = submodule.register_forward_hook(
        ...
        calibration_ids = np.arange(len(self._calibration_images))
        # optimize per layer
        for _ in range(self._num_calibration_steps):
            # reinit activations
            module_inputs  = {}
            module_outputs = {}
            # get batch idx
            ...
            with torch.no_grad():
                _ = self._teacher_module(self._calibration_images[batch_idx])
            #
            for submodule_name, submodule in self._modules_dict.items():
...
@PyTorchModifierYAML()
class AdaPruneMagnitudePruningModifier(AdaPruneBaseModifier, MagnitudePruningModifier):
    """
...
@PyTorchModifierYAML()
class AdaPruneGlobalMagnitudePruningModifier(AdaPruneBaseModifier, GlobalMagnitudePruningModifier):
    """
    This subclass of the AdaPruneBaseModifier implements 
...
@PyTorchModifierYAML()
class AdaPruneMovementPruningModifier(AdaPruneBaseModifier, MovementPruningModifier):
    """
...
@PyTorchModifierYAML()
class AdaPruneMFACPruningModifier(AdaPruneBaseModifier, MFACPruningModifier):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(AdaPruneMFACPruningModifier, self).__init__(
            params=params,
            init_sparsity=init_sparsity,
            final_sparsity=final_sparsity,
            start_epoch=start_epoch,
