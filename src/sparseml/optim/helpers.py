...
def print_staged_container(stage_container):
    for k, v in stage_container.items():
        print("===========")
        print(f"KEY:\n{k}\n")
        print("VALUES:\t")
...
def check_if_staged_recipe(container: Dict) -> Tuple[Dict, bool]:
    """
    Check whether the container has been generated by the standard recipe
    ...
    """
    staged_recipe = True
    stage_signature = "_stage"
    modifier_signature = "_modifier"

    if not any([stage_signature in key for key in container.keys()]):
        # if there are no stages in the container, this must be standard recipe, pass
        return container, not staged_recipe

    else:
        # parsing the container to create stage_container
        stage_container = {"global_variables": []}
        for key, val in container.items():
            # check whether the key of the staged recipe is a stage...
            if stage_signature in key:
                stage_container[key] = {}
                # parsing the single recipe (pertaining to a current stage)
                for recipe_key, recipe_val in val.items():
                    # check whether `recipe_key` contains `modifier_signature`
                    # e.g. `pruning_modifier` or `training_modifier ...
                    if modifier_signature in recipe_key:
                        if "modifiers" not in stage_container[key].keys():
                            stage_container[key]["modifiers"] = []
                        stage_container[key]["modifiers"] += recipe_val

                    # if `recipe_key` does not describe family of modifiers,
                    # this must be a stage variable.
                    else:
                        if "stage_variables" not in stage_container[key].keys():
                            stage_container[key]["stage_variables"] = []
                        stage_container[key]["stage_variables"].append(
                            {recipe_key: recipe_val}
                        )

            else:
                # ...if not, it must be a global variable
                stage_container["global_variables"].append({key: val})
        print_staged_container(stage_container)
        return stage_container, staged_recipe
...
def evaluate_recipe_yaml_str_equations(recipe_yaml_str: str) -> str:
    ...
    """
    container = load_recipe_yaml_str_no_classes(recipe_yaml_str)
    container = check_if_staged_recipe(container)
