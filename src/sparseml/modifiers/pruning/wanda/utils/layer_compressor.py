...
from sparseml.modifiers.pruning.wanda.utils.wandagpt import WandaGPT
...
class WandaLayerCompressor:
    ...
    def pre_compress_parallel(self, **kwargs) -> Dict:
        """
        Sets up the WrappedGPT objects for each compressible module,
        computes the statistics for each using the calibration data.

        :return: WrappedGPT objects for each module
        """
        ...
        # Run through the samples in order to compute statistics for each module
    ...
    def sequentially_compress(self, **kwargs):
        ...
        compression, we compute the statistics layer by layer instead of computing them
        ...
        for name in order:  # create WrappedGPT object for each compressible module
            ...
            # add WrappedGPT hook for current module
            ...
            for sample_idx in range(nsamples):
                ...
                for arg in passed_in_args:
                    if isinstance(kwargs[arg], List):
                        ...
                        passed_in_kwargs[arg] = kwargs[arg]
                # run layer, triggering WrappedGPT add_batch for current module
                self.layer(self.inputs[sample_idx], **passed_in_kwargs)
            handle.remove()

            _LOGGER.info(f"Compressing module {name} of layer {self.layer_index}")
            gpts.fasterprune(  # run WrappedGPT algorithm on current module
                self.args["sparsity"],
