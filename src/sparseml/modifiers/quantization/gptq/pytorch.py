...
from sparseml.modifiers.gptq.base import GPTQModifier
from sparseml.modifiers.gptq.utils.gptq_wrapper import GPTQWrapper
...
class GPTQModifierPyTorch(GPTQModifier):
    ...
    def on_initialize(self, state: "State", **kwargs) -> bool:
        ...
        if not self.quantize:
            raise ValueError(
                "Quantization must be enabled to use the GPTQModifier,"
                f" but got {self.quantize=}"
            )
    ...
    def initialize_compression(
        ...
        for idx, (name, layer) in enumerate(self.compressible_layers_.items()):
            ...
            compression_args = self._compression_arguments()
    ...
    def on_finalize(self, state: "State", **kwargs) -> bool:
        """
        disable the quantization observers used by the GPTQ algorithm
    ...
    def _compression_arguments(self):
        """
