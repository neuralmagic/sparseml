...
try:
    ...
    onnxruntime_gpu_installed = onnxruntime.get_device() == "GPU"
except Exception as err:
...
__all__ = [
    ...
    "onnxruntime",
    "onnxruntime_err",
    "onnxruntime_gpu_installed",
    "check_onnx_install",
    "check_onnxruntime_install",
    "check_onnxruntime_gpu_install",
    "require_onnx",
    "require_onnxruntime",
...
def check_onnxruntime_gpu_install(
    min_version: Optional[str] = _ORT_MIN_VERSION,
    max_version: Optional[str] = None,
    raise_on_error: bool = True,
) -> bool:
    """
    Check that the onnxruntime gpu package is installed.
    If raise_on_error, will raise an ImportError if it is not installed or
    the required version range, if set, is not installed.
    If not raise_on_error, will return True if installed with required version
    and False otherwise.
    ...
    if not onnxruntime_gpu_installed:
        if raise_on_error:
            raise RuntimeError("onnxruntime-gpu package not installed")
        return False

    return check_version("onnxruntime-gpu", min_version, max_version, raise_on_error)
...
def require_onnxruntime_gpu(
    min_version: Optional[str] = _ORT_MIN_VERSION, max_version: Optional[str] = None
):
    """
    Decorator function to require use of onnxruntime gpu.
    Will check that onnxruntime gpu package is installed and within the bounding
    ranges of min_version and max_version if they are set before calling
    the wrapped function.
    See :func:`check_onnxruntime_install` for more info.
    ...
    def _decorator(func):
        @functools.wraps(func)
        def _wrapper(*args, **kwargs):
            check_onnxruntime_gpu_install(min_version, max_version)
