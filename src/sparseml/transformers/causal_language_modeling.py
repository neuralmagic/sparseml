    ...
    config_overrides: Optional[str] = field(
        ...
        metadata={
            "help": (
                ...
                "trained from scratch. Example: n_embd=10,resid_pdrop=0.2,"
                "scale_attn_weights=false,summary_type=cls_index"
            )
        },
    )
...
class DataTrainingArguments:
    ...
    eval_split: Optional[str] = field(
        default="validation",
        metadata={
            "help": "Which dataset split (validation or test) to use when evaluating, "
            "i.e. when do_eval is used"
        },
    )
    one_shot: bool = field(
        default=False,
...
def main(**kwargs):
    ...
    model, teacher = SparseAutoModel.causal_language_modeling_from_pretrained_distil(
    ...
    # loading before tokenize_function
    ...
    if data_args.block_size is None:
        ...
        if block_size > 1024:
            _LOGGER.warning(
                ...
                "change that default value by passing --block_size xxx."
            )
            block_size = 1024
    else:
    ...
    if training_args.do_eval:
        if data_args.eval_split not in tokenized_datasets:
            raise ValueError(f"--do_eval requires a {data_args.eval_split} dataset")
        eval_dataset = lm_datasets[data_args.eval_split]
    ...
    # Evaluation
    if training_args.do_eval:
        _LOGGER.info(f"*** Evaluate on {data_args.eval_split} ***")
        ...
        trainer.log_metrics(f"eval_{data_args.eval_split}", metrics)
        trainer.save_metrics(f"eval_{data_args.eval_split}", metrics)
